...
// StringBetweenDelimeters will return the text between the given delimeters
func StringBetweenDelimeters(s, leftDelimeter, rightDelimeter string) string {
	i := strings.Index(s, leftDelimeter)
	if i >= 0 {
		j := strings.Index(s[i+1:], rightDelimeter)
		if j >= 0 && i+j+1 <= len(s) {
			return s[i+len(leftDelimeter) : i+j+1]
		}
	}
